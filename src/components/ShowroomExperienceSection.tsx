...
export default function ShowroomExperienceSection({
  ...
  return (
    ...
    >
      <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-[#E4D2BC] to-transparent" />
      ...
      <div className="relative mx-auto max-w-7xl">
        <div className="grid gap-12 lg:grid-cols-[0.92fr_1.08fr] lg:items-end">
          <div>
            <div className="mb-7 inline-flex items-center gap-3 rounded-full border border-[#E4D2BC] bg-white/55 px-4 py-2 shadow-sm">
              <span className="h-1.5 w-1.5 rounded-full bg-[#E0632C]" />
              <span className="text-[10px] font-semibold uppercase tracking-[0.28em] text-[#B14A1F]">
                {eyebrow}
              </span>
            </div>
              ...
              {infoItems.map((item, index) => {
                ...
                return (
                  <div key={`${item.label}-${item.value}`} className="rounded-2xl border border-[#E4D8C4] bg-white/58 px-4 py-4 shadow-[0_16px_35px_rgba(34,31,26,0.04)] backdrop-blur">
                    <Icon className="mb-4 h-4 w-4 text-[#E0632C]" aria-hidden="true" />
          ...
          <div className="lg:pl-10">
            <div className="border-l border-[#E4D2BC] pl-6 md:pl-8">
              ...
              <div className="mt-9 flex flex-col gap-3 sm:flex-row sm:items-center">
                ...
                <a
                  href={`#${id}-preview`}
                  className="inline-flex items-center justify-center gap-2 rounded-full border border-[#E4D2BC] bg-white/65 px-6 py-3 text-[10px] font-semibold uppercase tracking-[0.2em] text-[#221F1A] transition-all duration-300 hover:-translate-y-0.5 hover:border-[#E0632C] hover:text-[#B14A1F]"
                >
                  {secondaryCtaLabel}
        ...
        <div id={`${id}-preview`} className="mt-14 overflow-hidden rounded-[32px] border border-[#E4D2BC] bg-[#221F1A] shadow-[0_32px_80px_rgba(34,31,26,0.18)]">
          <div className="flex items-center justify-between gap-4 border-b border-white/10 bg-[#221F1A] px-5 py-4 text-[#F8F4EC] md:px-7">
            <div>
              <p className="text-[10px] uppercase tracking-[0.22em] text-[#E4D2BC]">Preview da experiência</p>
